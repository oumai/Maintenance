...
+ (CGFloat)calculateWidthWithText:(NSString *)text height:(CGFloat)height font:(UIFont *)font {
    
    CGRect  rect = [text boundingRectWithSize:CGSizeMake(0, height)
                                      options:NSStringDrawingUsesLineFragmentOrigin|NSStringDrawingUsesFontLeading
                                   attributes:@{NSFontAttributeName:font}
                                      context:nil];
    height = ceil(rect.size.width);
    return height;
}
...
+ (void)countdownWithTime:(int)time
    ...
    dispatch_source_set_event_handler(_timer, ^
    {
        if(timeout<=0) {
            ...
            NSString *strTime = [NSString stringWithFormat:@"发送验证码(%zd)", seconds];
            dispatch_async(dispatch_get_main_queue(), ^
            {
                //设置界面的按钮显示 根据自己需求设置
                going(strTime);
...
+ (NSString *)getCurrentID {
    KMMPerson *model = PERSON_INFO;
    return [NSString stringWithFormat:@"%@",model.Data.ID];
}
