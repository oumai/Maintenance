...
+ (NSDictionary *)objectClassInArray{
    return @{@"SecondCourseReply" : [secondReplyData class]};
}
...
- (double)commentTableViewHeight
{
    if (!_commentTableViewHeight) {
        ...
        if (self.SecondCourseReply.count > 0) {
            
            for (secondReplyData *commet in self.SecondCourseReply) {
...
- (double)commentHeight
{
    if (_commentHeight == 0) {
        ...
        string = [NSString stringWithFormat:@"%@：%@",self.NickName,self.Body];
